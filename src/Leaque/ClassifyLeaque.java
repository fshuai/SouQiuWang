...
public class ClassifyLeaque {
   ...
   public  static JSONObject getLeagueInfo(int legid,int start){
	    ResultSet rs=null; 
	    ResultSet rs1=null;
	    ...
	    try  
	       {   Class.forName("oracle.jdbc.driver.OracleDriver");
	         ...
	         if(legid!=11)
	          rs1=stmt.executeQuery("select count(*) from video,VIDEO_DYNAMIC where video.videoid in (select videoid from match_video where matchid = 1 ) and video.videoid=video_dynamic.videoid");
	         else   //国家队联赛
	          rs1=stmt.executeQuery("select count(*) from video,VIDEO_DYNAMIC where video.videoid in (select videoid from nationvideo) and video.videoid=video_dynamic.videoid");
	         ...
	         while(rs1.next())
	          ...
	          j1.put("total", total);
	          if(legid!=11)
	            rs=stmt.executeQuery("select * from(select rownum no,TempTable.* from(select video.videoid,videoaddr,videofile,videoname,videodatecome,totalduration,to_char(VIDEODATEUPLOAD,'YYYY-MM-DD') publishTime,CLICKNUM,videoorigin from video,VIDEO_DYNAMIC where video.videoid in (select videoid from match_video where matchid = " + legid + " ) and video.videoid=video_dynamic.videoid order by publishTime desc) TempTable) where no>=" + start + " and no<=" + start+len); 
	          else//国家队联赛
	            rs=stmt.executeQuery("select * from(select rownum no,TempTable.* from(select video.videoid,videoaddr,videofile,videoname,videodatecome,totalduration,to_char(VIDEODATEUPLOAD,'YYYY-MM-DD') publishTime,CLICKNUM,videoorigin from video,VIDEO_DYNAMIC where video.videoid in (select videoid from nationvideo) and video.videoid=video_dynamic.videoid order by publishTime desc) TempTable) where no>=" + start +" and no<=" + start+len);
	          int i=1;  
	    while(rs.next()&&i<=len){
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		   ClassifyLeaque t=new  ClassifyLeaque  ();
		   JSONObject list2=t.getLeagueInfo(2,0);
		   JSONArray a = null;
		try {
