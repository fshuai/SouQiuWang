...
public class getMore_snooker {
	static String baseAddress="http://202.114.18.70";
	 static int len;//  如果是排球可以一次性读出来，在后面让len=total就可以了，原来是固定住，指每次向其加载就是10个
	  ...
	  //select t.videoid,totalduration,videofile,videoaddr,videodatecome,videoname ,clickNum from VOLLEY_VIDEO t, VOLLEY_VIDEO_DYNAMIC t1 where t.videoid=t1.videoid
		  ...
		  try  
	       {   Class.forName("oracle.jdbc.driver.OracleDriver");
	          ...
	          rs1=stmt.executeQuery("select count(*)from  SNOOKER_VIDEO t ,SNOOKER_VIDEO_DYNAMIC t1 where t.videoid=t1.videoid");
	          while(rs1.next()){
	          ...
	          rs=stmt.executeQuery("select * from (select row_number() over(order by videodatecome desc) r,t.videoorigin, t.videoid,totalduration,videofile,videoaddr,videodatecome,videoname ,clickNum from SNOOKER_VIDEO t, SNOOKER_VIDEO_DYNAMIC t1 where t.videoid=t1.videoid)"
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		   getMore_snooker  t=new getMore_snooker ();
