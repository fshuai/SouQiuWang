...
public class getMore_TableTennis {
	static String baseAddress="http://202.114.18.70/TTS_Images/Image/";//乒乓球的基地址发生变化
	  ...
	  //select t.videoid,totalduration,videofile,videoaddr,videodatecome,videoname ,clickNum from VOLLEY_VIDEO t, VOLLEY_VIDEO_DYNAMIC t1 where t.videoid=t1.videoid
		  ...
		  try  
	       {   Class.forName("oracle.jdbc.driver.OracleDriver");
	          ...
	          rs1=stmt.executeQuery("select count(*)from  tts_VIDEO t ,tts_VIDEO_DYNAMIC t1 where t.videoid=t1.videoid");
	          while(rs1.next()){
	           total=Integer.valueOf( rs1.getObject(1).toString());//用于统计个数
	          }
	          j1.put("total", total);
	          rs=stmt.executeQuery("select * from (select row_number() over(order by videodatecome desc) r,t.videoorigin, t.videoid,totalduration,videofile,videoaddr,videodatecome,videoname ,clickNum from tts_VIDEO t, tts_VIDEO_DYNAMIC t1 where t.videoid=t1.videoid)"
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		   getMore_TableTennis t=new getMore_TableTennis();
