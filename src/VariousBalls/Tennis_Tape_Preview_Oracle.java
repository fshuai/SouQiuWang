...
public class Tennis_Tape_Preview_Oracle {
	/*********************网球的精彩镜头与视频预览*********************/
	int videoID;
	...
	String oraPwd="team";  
	 ...
	 Statement stmt=null;
 public Tennis_Tape_Preview_Oracle(int videoID){
...
public JSONArray getTapeInfo(){
 ...
 try{  
     ...
     rs=stmt.executeQuery("select * from Tennis_VIDEO_HIGHLIGHT where videoid="+videoID);
...
public JSONArray getPreviewInfo(){
 ...
 try{  
     ...
     conn=DriverManager.getConnection(oraUrl,oraUser,oraPwd);  
     stmt=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);  
     rs=stmt.executeQuery("select t3.framerate,t1.framefile,t2.startframe,t2.duration from Tennis_E2LSH_KEYFRAME t1,Tennis_E2LSH_SHOT t2,Tennis_VIDEO t3 where t2.videoid=t3.videoid and t2.shotid=t1.shotid and t3.videoid="+videoID);
    //int i=0;
...
public static void main(String args[]){
		int videoId=7046;
		Tennis_Tape_Preview_Oracle v=new Tennis_Tape_Preview_Oracle(videoId);
