...
public class Badmintain_Tape_Preview_Oracle {
	/********************羽毛的精彩镜头与视频预览*********************/
	int videoID;
	...
	String baseAddress="http://202.114.18.77:8885";//羽毛球的基地址发生变化
	 ...
	 Statement stmt=null;
 public Badmintain_Tape_Preview_Oracle(int videoID){
...
public JSONArray getTapeInfo(){
 ...
 try{  
     ...
     rs=stmt.executeQuery("select * from BADMINTAIN_VIDEO_HIGHLIGHT where videoid="+videoID);
...
public JSONArray getPreviewInfo(){
 ...
 try{  
     ...
     conn=DriverManager.getConnection(oraUrl,oraUser,oraPWD);  
     stmt=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);  
     rs=stmt.executeQuery("select t3.framerate,t1.framefile,t2.startframe,t2.duration from BADMINTAIN_E2LSH_KEYFRAME t1,BADMINTAIN_E2LSH_SHOT t2,BADMINTAIN_VIDEO t3 where t2.videoid=t3.videoid and t2.shotid=t1.shotid and t3.videoid="+videoID);
    //int i=0;
...
public static void main(String args[]){
		int videoId=7847;
		Badmintain_Tape_Preview_Oracle v=new Badmintain_Tape_Preview_Oracle(videoId);
