...
public JSONArray getTapeInfo(){
 ...
 try{  
     ...
     rs=stmt.executeQuery("select * from VIDEO_HIGHLIGHT where videoid="+videoID+"order by startframe asc");
...
public JSONArray getPreviewInfo(){
 ...
 try{  
     ...
     conn=DriverManager.getConnection(oraUrl,oraUser,oraPWD);  
     stmt=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);  
     rs=stmt.executeQuery("select t3.framerate,t1.framefile,t2.startframe,t2.duration from E2LSH_KEYFRAME t1,E2LSH_SHOT t2,VIDEO t3 where t2.videoid=t3.videoid and t2.shotid=t1.shotid and t3.videoid="+videoID+"order by startframe asc");
    //int i=0;
