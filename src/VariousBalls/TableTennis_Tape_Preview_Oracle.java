...
public class TableTennis_Tape_Preview_Oracle {
	/********************乒乓球的精彩镜头与视频预览*********************/
	int videoID;
	JSONObject JTape_Preview=new JSONObject();
	String baseAddress="http://202.114.18.70/TTS_Images/Image/";//乒乓球的基地址发生变化
	 ...
	 Statement stmt=null;
 public TableTennis_Tape_Preview_Oracle(int videoID){
...
public JSONArray getTapeInfo(){
 ...
 try{  
     ...
     rs=stmt.executeQuery("select * from tts_VIDEO_HIGHLIGHT where videoid="+videoID);
...
public JSONArray getPreviewInfo(){
 ...
 try{  
     ...
     conn=DriverManager.getConnection(oraUrl,oraUser,oraPWD);  
     stmt=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);  
     rs=stmt.executeQuery("select t3.framerate,t1.framefile,t2.startframe,t2.duration from tts_E2LSH_KEYFRAME t1,tts_E2LSH_SHOT t2,tts_VIDEO t3 where t2.videoid=t3.videoid and t2.shotid=t1.shotid and t3.videoid="+videoID);
    //int i=0;
...
public static void main(String args[]){
		int videoId=2413;
		TableTennis_Tape_Preview_Oracle v=new TableTennis_Tape_Preview_Oracle(videoId);
