...
public class videoTape_Preview_Oracle {
	int videoID;
	...
	String baseAddress="http://202.114.18.70";
	 ...
	 Statement stmt=null;
public videoTape_Preview_Oracle(int videoID){
...
public JSONArray getTapeInfo(){
 ...
 try{  
     ...
     rs=stmt.executeQuery("select * from VOLLEY_VIDEO_HIGHLIGHT where videoid="+videoID+"order by startframe asc");
     while(rs.next()){
...
public JSONArray getPreviewInfo(){
 ...
 try{  
     ...
     conn=DriverManager.getConnection(oraUrl,oraUser,oraPWD);  
     stmt=conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);  
     rs=stmt.executeQuery("select t3.framerate,t1.framefile,t2.startframe,t2.duration from VOLLEY_E2LSH_KEYFRAME t1,VOLLEY_E2LSH_SHOT t2,volley_VIDEO t3 where t2.videoid=t3.videoid and t2.shotid=t1.shotid and t3.videoid="+videoID+"order by startframe asc");
    //int i=0;
...
public static void main(String args[]){
		int videoId=6893;
	videoTape_Preview_Oracle v=new videoTape_Preview_Oracle(videoId);
